 ...
 *  
 *  http://openbd.org/
 *  $Id: cfCOMPONENT.java 2374 2013-06-10 22:14:24Z alan $
 */
...
public class cfCOMPONENT extends cfTag implements Serializable {
	...
	public java.util.Map[] getAttInfo(){
		return new java.util.Map[] {
				...
				createAttInfo("DISPLAYNAME", "The name that is used to describe this CFC when displayed in meta data", "", false ),
				createAttInfo("HINT", "A small description for this component", "", false ),
				createAttInfo("EXTENDS", "The CFC that this instance will extend", "", false ),
				createAttInfo("IMPLEMENTS", "The CFC of the CFINTERFACE that this CFC must implement", "", false ),
				createAttInfo("TYPE", "The type of this CFC; component, interface or abstract", "component", false ),
				createAttInfo("OUTPUT", "Controls whether or not this CFC can generate output", "YES", false ),
				createAttInfo("WSDLFILE", "The file path of a well formatted WSDL file to use instead of the generated one", "", false ),
				createAttInfo("STYLE", "When the CFC is used as web services determines the encoded style; rpc, document, document-wrapped", "rpc", false ),
		};
	...
	public void addPropertyMetaData( cfStructData metaData ) {
		// TODO: check for duplicate property names? test serialization (see cfARGUMENT/cfFUNCTION)
		try {
			propertyMetadata.addElement( metaData );
	...
	public void addFunctionMetaData( cfStructData metaData ) {
		try {
			functionMetadata.addElement( metaData );
		} catch ( cfmRunTimeException ignore ) {
			// not going to happen
		}
	}
    ...
    if ( componentData != null ) {		
		...
		if ( ( renderResults != null ) &&
				(( componentData == null ) || 
				string.convertToBoolean( getConstant( "OUTPUT" ), true ) ) ) {
		...
		for ( int i = 0; i < implementsList.size(); i++ ) {
			...
			for ( int j = 0; j < keys.length; j++ ) {
				...
				if ( !componentData.containsKey( key ) ) {
					throw newBadFileException( "Invalid Attribute",
						"Component must implement function \"" + key + "\" from interface \"" + interfaceName + "\"" );
				}
			}
		}
	...
	/**
	 * The supercomponent reference in a UDF is a runtime attribute, so duplicate UDFs in the variables
	 * scope, setting the supercomponent reference, and place the duplicate copy back in the variables
	 * and CFC "this" scope.
	 */
			...
			if ( superValue.getDataType() == cfData.CFUDFDATA ) {
				...
				if ( !componentData.containsKey( key ) ) {
					...
					if ( ((userDefinedFunction)superValue).isAbstract() ) {
						throw newBadFileException( "Invalid Attribute",
							"Component must implement inherited abstract function \"" + key + "\" or specify TYPE=\"abstract\"" );
					}
