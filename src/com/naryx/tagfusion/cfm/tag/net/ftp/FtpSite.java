 ...
 *  
 *  http://openbd.org/
 *  $Id: FtpSite.java 2275 2012-09-04 00:20:26Z alan $
 */
...
public class FtpSite extends FtpClose {
	private static final long serialVersionUID = 1L;

	public FtpSite() {
		min = 2; max = 3;
		setNamedParams( new String[]{ "ftpdata", "cmd", "stoponerror" } );
	}
	...
	public java.util.Map getInfo(){
		return makeInfo(
				"ftp", 
				"Send a site specific command to the ftp server", 
				ReturnType.BOOLEAN );
	}
	...
	public cfData execute(cfSession _session, cfArgStructData argStruct) throws cfmRunTimeException {
		...
		String	cmd	= getNamedStringParam(argStruct, "cmd", null );
		if ( cmd == null )
			throwException(_session, "missing 'cmd' parameter");

		try{
			ftpdata.lock();
			
			boolean bResult = ftpdata.siteCmd(cmd);
			if ( soe && !ftpdata.isSucceeded() )
				throwException( _session, ftpdata.getErrorText() );
			
			return cfBooleanData.getcfBooleanData( bResult );
