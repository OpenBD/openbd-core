 ...
 *  
 *  http://openbd.org/
 *  $Id: cfApplicationData.java 2331 2013-02-25 20:07:43Z alan $
 */
...
public class cfApplicationData extends cfStructExpireData implements java.io.Serializable {
  ...
  private	SessionStorageInterface	SessionStorage = null;
	...
	public final String  getLoginStorage() { return loginStorage; }
	public final int     getTotalSessions(){ return SessionStorage.size(); }
	public final boolean isSessionEnabled(){ return SessionStorage.getType() != SessionStorageFactory.SessionEngine.NONE; }
	public final boolean isJ2EESessionEnabled(){ return SessionStorage.getType() == SessionStorageFactory.SessionEngine.J2EE; }
	public final boolean isClientEnabled(){ return bClient; }
	public final boolean isCookiesWorking(){ return bCookiesWorking; }
	
	public final String		getDataSource() { return datasource; }
	...
	public void onRequestStart( cfSession Session, boolean  bJ2EESessionManagement, long applicationTimeOut,
						long sessionTimeOut, boolean setClientCookies, boolean setDomainCookies,
						boolean bSessionManagement, boolean bClientManagement, String clientStorage,
						String loginStorage, String applicationCfcPath, String scriptProtect, boolean secureJson, String secureJsonPrefix,
						String datasource, String sessionStorage, cfComponentData applicationCfc ) throws cfmRunTimeException {
		...
		if ( bClient || 
				SessionStorage.getType() == SessionStorageFactory.SessionEngine.INTERNAL ||
				SessionStorage.getType() == SessionStorageFactory.SessionEngine.MONGO ||
				SessionStorage.getType() == SessionStorageFactory.SessionEngine.MEMCACHED
				) {
	...
	/**
	 * Called at the end of every request.  This is an opportunity for the client and any
	 * session storage engine to page their results out to a back end
	 * 
	 * @param Session
	...
	private void applyScriptProtection( cfSession _Session, int _scope ){
		...
		if ( scopeData != null && scopeData.getDataType() == cfData.CFSTRUCTDATA ){
			...
			for ( int i = 0; i < keys.length; i++ ){
				...
				if ( valueData.getDataType() == cfData.CFSTRINGDATA ){
					...
					if ( value.length() != origLen ){ 
						if ( _scope == variableStore.COOKIE_SCOPE ){ // special handling for cookie data
							( (cfCookieData) scopeData ).updateCookie( nextKey, value );
						}else{
							scopeData.setData( nextKey, new cfStringData( value ) );
						}
					}
				}
			}
