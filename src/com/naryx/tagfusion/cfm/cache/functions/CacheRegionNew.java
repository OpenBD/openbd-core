 ...
 *  
 *  http://openbd.org/
 *  $Id: CacheRegionNew.java 2131 2012-06-27 19:02:26Z alan $
 */
...
public class CacheRegionNew extends functionBase {
	
	private static final long serialVersionUID = 1;

	public CacheRegionNew() { 
		min = 1; max = 3;
		setNamedParams( new String[]{ "region", "props", "throwonerror" } );
	}
	...
	public String[] getParamInfo(){
		return new String[]{
			...
			"if the region exists, throw an exception (default true)"
		};
	}
	...
	public cfData execute(cfSession _session, cfArgStructData argStruct ) throws cfmRunTimeException {
		...
		if ( CacheFactory.isCacheEnabled(region) ){
			if ( getNamedBooleanParam(argStruct, "throwonerror", false) )
				throwException( _session, "'" + region + "' already exists region" );
			...
			try{
				CacheFactory.createCacheEngine( region, props );
			}catch(Exception e){
				throwException( _session, e.getMessage() );
			}
