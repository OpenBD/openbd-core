...
/**
 * Converts unidentifiable non-array data types to/from their BD equivalents.
 */
public class UnknownConverter {
	public UnknownConverter() {}
	...
	public static boolean toBDType(ObjectWrapper wrapper, cfSession session) throws cfmRunTimeException {
		// Cannot do anything with arrays
		if (wrapper.value.getClass().isArray())
			return false;

		// Present it as a cfJavaObjectData
		wrapper.value = new cfJavaObjectData(wrapper.value);
		;
		return true;
	}
