...
/**
 * This does everything tableSource does except that it
 * only returns rows that meet the condition criteria
 */
...
class filteredTableSource extends tableSource{

	condition filter; 
	
	filteredTableSource( Map<String, cfQueryResultData> _tables, condition _filter, List<cfData> _pData ){
	...
	boolean hasNext() throws cfmRunTimeException{
		...
		while ( super.hasNext() ){
			super.nextRow();
			if ( filter.evaluate( currentRow, pData ) ){
				return true;
			}
		}
		return false;
