...
public class arrayValueCount extends functionBase {
	private static final long serialVersionUID = 1L;

	public arrayValueCount() {
		min = max = 2;
	...
	public cfData execute(cfSession _session, cfArgStructData argStruct) throws cfmRunTimeException {
		...
		if (data.getDataType() == cfData.CFARRAYDATA) {
			...
			for ( int x=1; x <= array.size(); x++ ){
				...
				if ( cfData.isSimpleValue(element) && cfData.isSimpleValue(search) && element.equals(search) )
					total++;
