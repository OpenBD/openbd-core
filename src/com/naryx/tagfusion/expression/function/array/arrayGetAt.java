...
public class arrayGetAt extends functionBase {
	...
	public cfData execute(cfSession _session, cfArgStructData argStruct) throws cfmRunTimeException {
		...
		if (array.getDataType() == cfData.CFARRAYDATA) {
			...
			return ((cfArrayData) array).getElement( indx );
		}	else if ( array.getDataType() == cfData.CFBINARYDATA ) {
			if ( indx > ((cfBinaryData)array).getLength() || indx <= 0 )
				throwException(_session, "the index is out of range.  This array has " + ((cfBinaryData)array).getLength() + " bytes");
			
			return new cfNumberData( ((cfBinaryData)array).getByteAt(indx-1) );
		} else
