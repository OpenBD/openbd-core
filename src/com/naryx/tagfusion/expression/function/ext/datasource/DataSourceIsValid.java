...
public class DataSourceIsValid extends functionBase {
	private static final long serialVersionUID = 1L;

	public DataSourceIsValid() {
		min = max = 1;
	...
	public java.util.Map getInfo(){
		return makeInfo(
				"query", 
				"Checks to see if a given datasource has been previously registered using DataSourceCreate()", 
				ReturnType.BOOLEAN );
	}
	
	public cfData execute(cfSession _session, cfArgStructData argStruct ) throws cfmRunTimeException {
		String datasource = getNamedStringParam( argStruct, "datasource", null );
		return cfBooleanData.getcfBooleanData( cfDataSourceDetailsFactory.isRegistered(datasource) );
	}
}
