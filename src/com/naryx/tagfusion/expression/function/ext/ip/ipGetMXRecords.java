 ...
 *  
 *  http://openbd.org/
 *  $Id: ipGetMXRecords.java 1983 2012-03-16 00:02:45Z alan $
 */
...
public class ipGetMXRecords extends functionBase {
	private static final long serialVersionUID = 1L;
	
	public ipGetMXRecords() {
		min = 1; max = 2;
	...
	public String[] getParamInfo(){
		return new String[]{
				"domain - the name of the server you wish to get the MX records for",	
		...
		try {
			...
			if ( records != null ){
				for ( int x=0; x < records.length; x++ ){
					String tmp	= ((MXRecord)records[x]).getTarget().toString();
					if ( tmp.endsWith(".") )
						tmp 	= tmp.substring(0,tmp.length()-1);
					array.addElement( new cfStringData( tmp ) );
				}
			}
