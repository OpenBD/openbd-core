...
public class structKeyExists extends functionBase {
	...
	public cfData execute(cfSession _session, cfArgStructData argStruct) throws cfmRunTimeException {
		...
		String key = getNamedStringParam(argStruct, "key", "");
		boolean result = false;

		if (data.getDataType() == cfData.CFQUERYRESULTDATA) {
			result = ((cfQueryResultData) data).getColumnIndexCF(key) > 0;
		} else if (data.isStruct()) {
			result = ((cfStructData) data).containsKey(key);
			if (result) {
				cfData value = ((cfStructData) data).getData(key);
				result = ((value != null) && (value != CFUndefinedValue.UNDEFINED));
			}
		} else if (data.getDataType() == cfData.CFJAVAOBJECTDATA) {
			try {
				((cfJavaObjectData) data).getJavaData(key);
				result = true;
