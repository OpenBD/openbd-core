 ...
 *  
 *  $Id: FileFunction.java 1638 2011-07-31 16:08:50Z alan $
 */

package com.bluedragon.search.index.file;
...
public class FileFunction extends CustomFunction {
	private static final long	serialVersionUID	= 1L;

	public FileFunction(){
		String[] p = new String[]{"collection","key","title","summary","author","category","categorytree","urlpath","custommap","query"};
		min = 2;
		max = p.length;
		setNamedParams( p );
	}
	...
	public java.util.Map getInfo(){
		return makeInfo(
				"search", 
				"Inserts/Updates a file into the collection.  The key is the unique file for each document.  Each field in the document can be searched against.  " +
	...
	public cfData execute(cfSession _session, cfArgStructData argStruct) throws cfmRunTimeException {
		...
		if ( query != null )
			return indexFileQuery( collection, query, _session, custommap, argStruct );
		else
			return indexFile( collection, _session, custommap, argStruct );
	}
	 ...
	 */
	private cfData indexFile(Collection collection, cfSession _session, cfStructData custommap, cfArgStructData argStruct) throws cfmRunTimeException  {
		...
		try{
			DocumentWrap	doc	= crawlfactory.crawlFile( getNamedStringParam(argStruct, "urlpath", null ), file);
			if ( doc == null )
				throw new Exception("File not valid or unhandled extension:"  + file.toString() );
	...
	private cfData indexFileQuery(Collection collection, cfQueryResultData query, cfSession _session, cfStructData custommap, cfArgStructData argStruct) throws cfmRunTimeException {
		cfStructData	sd	= new cfStructData();
		int totalDocs = 0;
		...
		try{
			...
			while (query.nextRow()) {
				...
				DocumentWrap	doc	= crawlfactory.crawlFile( urlpath, file);
				if ( doc == null ){
